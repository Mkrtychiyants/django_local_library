...
class AuthorCreateTest(TestCase):
    ...
    def test_redirect_if_not_logged_in(self):
        ...
        self.assertRedirects(resp, '/accounts/login/?next=/catalog/authors/create/')
    def test_redirect_if_logged_in_but_not_correct_permission(self):
        login = self.client.login(username='testuser1', password='12345')
        resp = self.client.get(reverse('author_create' )
        self.assertRedirects(resp, '/accounts/login/?next=/catalog/authors/create/')
    ...
    def test_form_date_of_death_initially_set_to_expected_date(self):         
        ...
        resp = self.client.get(reverse('author_create' )
        self.assertEqual(resp.status_code,200)
        exp_date=datetime.date(2016,12,10)
        response_date=rest.context['form'].initial['date_of_death']
        response_date=datetime.datetime.strptime(response_date,"%Y-%m-%d").date()
        self.assertEqual(response_date,exp_date)
